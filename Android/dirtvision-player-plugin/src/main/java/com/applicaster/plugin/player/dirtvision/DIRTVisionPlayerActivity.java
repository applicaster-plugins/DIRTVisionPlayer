...
import com.applicaster.plugin.player.dirtvision.ui.util.SimpleLifecycleCallbacks;
...
public class DIRTVisionPlayerActivity extends JWPlayerActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        activityLifecycleCallbacks = new SpecificActivityLifecycleCallbacks(this, new SimpleLifecycleCallbacks() {
            @Override
    ...
    private void setupNextValidation() {
        if (playable != null && playable.isLive()) {
            long nextValidation = PlayerUtils.getNextValidationTime(liveConfig);
            long now = PlayerUtils.getCurrentTime();
    ...
    private void presentValidationPlugin() {
        if (playable != null &&
                ((playable.isLive() && PlayerUtils.isLiveValidationNeeded(liveConfig))
                || PlayerUtils.isValidationNeeded(playable))) {
    ...
    private void setupNextRefresh() {
        ...
        long nextUpdate = PlayerUtils.getProgramFinishTime(liveConfig);
        long now = PlayerUtils.getCurrentTime();
